        ...
        private void Update()
        {
            ...
            if (seed != 0)
            {
				...
				if (setIntensity)
				{
					...
					(
						IntensityModifier * intensityModifier2 * Mathf.PerlinNoise(seed + Time.time, seed + 1 + Time.time),
						IntensityMaxRange.Minimum, 
						IntensityMaxRange.Maximum
					);
				...
				//float y = Mathf.PerlinNoise(seed + 2 + Time.time * 2, seed + 3 + Time.time * 2) - 0.5f;
				//float z = Mathf.PerlinNoise(seed + 4 + Time.time * 2, seed + 5 + Time.time * 2) - 0.5f;

				float x = Mathf.PerlinNoise(Random.Range(-0.1f, 0.1f), Random.Range(-0.1f, 0.1f)) * Time.deltaTime;
				float y = Mathf.PerlinNoise(Random.Range(-0.1f, 0.1f), Random.Range(-0.1f, 0.1f)) * Time.deltaTime;
				float z = Mathf.PerlinNoise(Random.Range(-0.1f, 0.1f), Random.Range(-0.1f, 0.1f)) * Time.deltaTime;
