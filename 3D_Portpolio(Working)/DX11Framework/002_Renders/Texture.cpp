...
D3D11_TEXTURE2D_DESC Texture::ReadPixels( ID3D11Texture2D * src, DXGI_FORMAT readFormat, vector<Vector4>* pixels )
{
	D3D11_TEXTURE2D_DESC srcDesc;
	src->GetDesc( &srcDesc );
	...
	for ( UINT y = 0; y < desc.Height; y++ )
	{
		for ( UINT x = 0; x < desc.Width; x++ )
		{
			...
			pixels->push_back( Vector4( a, b, g, r ) );
		}
	}
...
D3D11_TEXTURE2D_DESC Texture::ReadPixels( DXGI_FORMAT readFormat, vector<Vector4>* pixels )
{
	...
	: srv( NULL )
{
	...
	// Texture2DArray »ý¼º
	{
		...
		desc.ArraySize			= ( UINT )names.size();
	...
	for ( UINT i = 0; i < textures.size(); i++ )
	{
		for ( UINT level = 0; level < textureDesc.MipLevels; level++ )
		{
			...
			D3D::GetDC()->Map( textures[ i ], level, D3D11_MAP_READ, 0, &subResource );
			{
				...
				(
					textureArray,
					D3D11CalcSubresource( level, i, textureDesc.MipLevels ),
					NULL,
					subResource.pData,
	...
	// Create SRV
	{
		...
		desc.Texture2DArray.ArraySize		= ( UINT )names.size();
