...
{
	mainCamera->Update		();
	mainCamera->GetMatrix	( &perFrame.View );
	mainCamera->GetPosition	( &perFrame.ViewPosition );
	mainCamera->GetForward	( &perFrame.ViewDirection );
	
	perFrame.Time = Time::Get()->Running();

	for ( pair<Shader*, CBuffer*> frame : perFrameMap )
		frame.second->Change();
