...
void D3D::SetGpuInfo()
{
	...
	{
		bool bCheck = true;
		bCheck &= ( displayModeList[ i ].Width == d3dDesc.Width );
		bCheck &= ( displayModeList[ i ].Height == d3dDesc.Height );
	...
	d3dDesc.gpuDescription	= adapterDesc.Description;
...
{
	if ( rtv == NULL ) rtv = renderTargetView;
	if ( dsv == NULL ) dsv = depthStencilView;

	D3D::GetDC()->OMSetRenderTargets( 1, &rtv, dsv );
}

void D3D::Clear( XMFLOAT4 color, ID3D11RenderTargetView * rtv, ID3D11DepthStencilView * dsv )
{
	if ( rtv == NULL ) rtv = renderTargetView;
	if ( dsv == NULL ) dsv = depthStencilView;
	...
	deviceContext->ClearRenderTargetView( rtv, colorArr );
