...
void Patch::BuildCenterIndices()
{
	for ( WORD tessLevel = 0; tessLevel <= 6; tessLevel++ )
	{
		...
		for ( WORD z = 0 + t, z0 = t; z < width; z += t, z0 += t )
		{
			...
			{
				indices.push_back( ( z0 + 0 ) * ( width + 1 ) + ( x0 + 0 ) );
				indices.push_back( ( z0 + 0 ) * ( width + 1 ) + ( x0 + t ) );
				indices.push_back( ( z0 + t ) * ( width + 1 ) + ( x0 + 0 ) );
		...
		centerIndexCount[ t ] = indices.size();
	}
}
...
void Patch::BuildTopEdges()
{
	...
	for ( WORD i = 0; i < 6; i++ )
	{
		...
		edgeIndexCount[ NeighborDir::Top ][ key ] = indices.size();
		indices.clear();
	}
	...
	for ( WORD i = 0; i < 6; i++ )
	{
		...
		for ( WORD x = 0 + t1, x0 = t1; x < width; x += t1, x0 += t1 )
		{
			...
			indices.push_back( ( z0 + 0 ) * ( width + 1 ) + ( x0 + t1 ) );
...
void Patch::BuildLeftEdges()
{
	...
	for ( WORD i = 0; i < 6; i++ )
	{
		...
		edgeIndexCount[ NeighborDir::Left ][ key ] = indices.size();
		indices.clear();
	}
	...
	for ( WORD i = 0; i < 6; i++ )
	{
		...
		for ( WORD z = 0 + t1, z0 = t1; z < width; z += t1, z0 += t1 )
		{
			...
			indices.push_back( ( z0 + t1 ) * ( width + 1 ) + ( x0 + 0  ) );
