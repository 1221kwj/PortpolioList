...
ModelMeshPart::ModelMeshPart() : pass( 0 ) {}
...
void ModelMeshPart::Binding()
{
	...
	//1. Vertex Buffer
	{
		...
		desc.ByteWidth	= sizeof( VTNTB ) * ( UINT )vertices.size();
...
void ModelMeshPart::Render()
{
	...
	material->GetShader()->GetAsScalar( "BoneIndex" )->SetInt( ( int )boneIndex );

	D3D::GetDC()->DrawIndexed( ( UINT )indices.size(), 0, 0 );;
}

void ModelMeshPart::Clone( ModelMeshPart** clone )
{
